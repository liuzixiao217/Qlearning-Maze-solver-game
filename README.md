...
Open the project in a Python environment (e.g., Google Colab).
Run the code to install dependencies and initialize the game.
Click the Step (1 step) button to start the game and let the agent explore.
Use Train/Play to switch between training and play modes.
Click Reset to restart the game with a new maze.
...
Q-table: A 2D array initialized to 0, where rows represent states (maze positions) and columns represent actions (movement directions).
ε-greedy strategy: Balances exploration (random actions) and exploitation (optimal actions based on Q-values).
Q-value update: Uses the formula:Q(s,a)=Q(s,a)+α×[r+γ×maxQ(s′,a′)−Q(s,a)]where α is the learning rate, γ is the discount factor, r is the immediate reward, and maxQ(s′,a′) is the maximum Q-value of the next state.
